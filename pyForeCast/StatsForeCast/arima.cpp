...
namespace arima {
	...
	using tcb::span;
	namespace {
		template <typename T> span<T> make_span(py::array_t<T> &array) {
		...
		template <typename T>
		span<const T> make_cspan(const py::array_t<T> &array) {
			// py::ssize_t to size_t is safe because the size of an array is non-negative
			return { array.data(), static_cast<size_t>(array.size()) };
		}
	} // namespace

	void partrans(const uint32_t p, const span<const double> rawv,
		const span<double> newv) {
		assert(p <= rawv.size());
		assert(p <= newv.size());
	...
	std::tuple<py::array_t<double>, py::array_t<double>>
		...
		if (trans) {
			...
			if (msp > 0) {
				partrans(msp, params_in.subspan(v), span(params).subspan(v));
			}
		}
	...
	void inclu2(const std::vector<double> &xnext, std::vector<double> &xrow,
		double ynext, span<double> d, std::vector<double> &rbar,
	...
	void init(py::module_ &m) {
		...
		arima.doc() = "Stats Forebast Arima module";
