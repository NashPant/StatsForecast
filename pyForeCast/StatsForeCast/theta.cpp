...
namespace theta {
	...
	double calc(const Eigen::Ref<const VectorXd> &y,
		...
		// Use std copy here as ranges are c++20
		std::copy(init_states.begin(), init_states.end(), states.row(0).begin());
	...
	void init(py::module_ &m) {
		py::module_ theta = m.def_submodule("stats-theta");
		theta.doc() = "Stats Forebast theta module";
