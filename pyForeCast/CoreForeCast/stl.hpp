...
template <typename T>
bool est(const T *y, size_t n, size_t len, int ideg, T xs, T *ys, size_t nleft, size_t nright, T *w,
         bool userw, const T *rw) {
    auto range = ((T)n) - 1.0;
...
template <typename T>
void ess(const T *y, size_t n, size_t len, int ideg, size_t njump, bool userw, const T *rw, T *ys,
         T *res) {
    if (n < 2) {
        ys[0] = y[0];
        return;
...
template <typename T> void fts(const T *x, size_t n, size_t np, T *trend, T *work) {
    ma(x, n, np, trend);
...
template <typename T>
void ss(const T *y, size_t n, size_t np, size_t ns, int isdeg, size_t nsjump, bool userw, T *rw,
        T *season, T *work1, T *work2, T *work3, T *work4) {
...
void onestp(const T *y, size_t n, size_t np, size_t ns, size_t nt, size_t nl, int isdeg, int itdeg,
            int ildeg, size_t nsjump, size_t ntjump, size_t nljump, size_t ni, bool userw, T *rw,
            T *season, T *trend, T *work1, T *work2, T *work3, T *work4, T *work5) {
...
template <typename T>
void stl(const T *y, size_t n, size_t np, size_t ns, size_t nt, size_t nl, int isdeg, int itdeg,
         int ildeg, size_t nsjump, size_t ntjump, size_t nljump, size_t ni, size_t no, T *rw,
         T *season, T *trend) {
    if (ns < 3) {
    ...
    while (true) {
        onestp(y, n, np, ns, nt, nl, isdeg, itdeg, ildeg, nsjump, ntjump, nljump, ni, userw, rw,
               season, trend, work1.data(), work2.data(), work3.data(), work4.data(), work5.data());
        k += 1;
        if (k > no) {
            break;
        }
...
template <typename T> T var(const std::vector<T> &series) {
...
template <typename T> T strength(const std::vector<T> &component, const std::vector<T> &remainder) {
    std::vector<T> sr;
...
template <typename T> class StlResult {
  public:
    ...
    inline T seasonal_strength() { return strength(seasonal, remainder); }

    inline T trend_strength() { return strength(trend, remainder); }
};

template <typename T = float> class StlParams {
...
template <typename T = float> StlParams<T> params() { return StlParams<T>(); }

template <typename T> StlResult<T> StlParams<T>::fit(const T *y, size_t n, size_t np) {
    if (n < 2 * np) {
    ...
    auto res =
            StlResult<T>{std::vector<T>(n), std::vector<T>(n), std::vector<T>(), std::vector<T>(n)};
    ...
    stl(y, n, newnp, newns, nt, nl, isdeg, itdeg, ildeg, nsjump, ntjump, nljump, ni, no,
        res.weights.data(), res.seasonal.data(), res.trend.data());
...
template <typename T> StlResult<T> StlParams<T>::fit(const std::vector<T> &y, size_t np) {
